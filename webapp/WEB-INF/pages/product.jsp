...
<head>
    ...
    <link rel="stylesheet" href="<c:url value="/css/style-product.css"/>">
...
<div class="container-fluid menu-cap">
    ...
    <div class="btn-group btn-group-sm pull-right menu">
        ...
        <sec:authorize access="hasRole('USER')">
            <a href="<c:url value="/user/basket_deals_list/${0}/${'product'}"/>"
            ...
            <sec:authorize access="hasRole('ADMIN')">
                <table>
                    <tr>
                        ...
                        <td><h3 class="info">${pageContext.request.userPrincipal.name}</h3></td>
                    </tr>
                </table>
            </sec:authorize>
...
<div class="container container-bc">
    <ol class="breadcrumb">
        ...
        <sec:authorize access="hasRole('ADMIN')">
            <c:if test="${page eq 'list_product' || page eq 'create'}">
                <li class="li-bc"><a href="<c:url value="/list_product/${idPC}/${namePC}"/>" class="a-bc">назад</a></li>
            </c:if>
        </sec:authorize>
        <c:if test="${page eq 'basket' || page eq 'deal' || page eq 'user_deals'}">
...
<div class="container-fluid ">
    <div class="row">
        <div class="col-sm-6 prod-col">
            <div class="thumbnail">
                <div class="prod-img">
                    <a href="#"><img src="<c:url value="/photo/${product.photo.id}"/>"
                                     alt="изображение отсутствует"></a>
                </div>
            </div>
        </div>
        <div class="col-sm-6 prod-col">
            <div class="thumbnail prod-thum">
                <h1><c:out value="${product.name}"/></h1>
                <table>
                    <c:if test="${product.color ne '' && product.color ne ' '}">
                        <tr>
                            <td><p class="td-1">цвет:</p></td>
                            <td class="td-2"><p><c:out value="${product.color}"/></p></td>
                        </tr>
                    </c:if>
                    <c:if test="${product.size ne '' && product.size ne ' '}">
                        <tr>
                            <td><p class="td-1">размер:</p></td>
                            <td><p class="td-2"><c:out value="${product.size}"/></p></td>
                        </tr>
                    </c:if>
                    <c:if test="${product.season ne '' && product.season ne ' '}">
                        <tr>
                            <td><p class="td-1">сезон:</p></td>
                            <td class="td-2"><p><c:out value="${product.season}"/></p></td>
                        </tr>
                    </c:if>
                    <c:if test="${product.zastezhka ne ''&& product.zastezhka ne ' '}">
                        <tr>
                            <td><p class="td-1">застёжка:</p></td>
                            <td><p class="td-2"><c:out value="${product.zastezhka}"/></p></td>
                        </tr>
                    </c:if>
                    <c:if test="${product.length ne '' && product.length ne ' '}">
                        <tr>
                            <td><p class="td-1">длина:</p></td>
                            <td><p class="td-2"><c:out value="${product.length}"/></p></td>
                        </tr>
                    </c:if>
                    <c:if test="${product.sleeve ne '' && product.sleeve ne ' '}">
                        <tr>
                            <td><p class="td-1">рукав:</p></td>
                            <td><p class="td-2"><c:out value="${product.sleeve}"/></p></td>
                        </tr>
                    </c:if>
                    <c:if test="${product.material ne '' && product.material ne ' '}">
                        <tr>
                            <td><p class="td-1">материал:</p></td>
                            <td><p class="td-2"><c:out value="${product.material}"/></p></td>
                        </tr>
                    </c:if>
                </table>
                <c:if test="${product.description ne '' && product.description ne ' '}">
                    <div class="caption prod-desc">
                        <p class="prod-d1">описание:</p>
                        <p class="prod-d2"><c:out value="${product.description}"/></p>
                    </div>
                </c:if>
                <sec:authorize access="isAnonymous()">
                    <tr>
                        <td>
                            <p class="anonim">для возможности совершать покупки зарегистрируйтесь</p>
                        </td>
                    </tr>
                </sec:authorize>
                ...
                <div class="btn-group modal-group">
                    <sec:authorize access="hasRole('USER')">
                        <c:if test="${page ne 'basket'&& page ne 'user_deals'}">
                            <a class="btn active btn-p"
                               href="<c:url value="/user/add_in_basket/${product.id}/${product.name}/${product.photo.id}"/>">в
                                корзину</a>
                        </c:if>
                    </sec:authorize>
                    <c:if test="${page ne 'deal'}">
                        <sec:authorize access="hasRole('ADMIN')">
                            <a class="btn active btn-w"
                               href="<c:url value="/admin/change_delete_product/${product.id}"/>"
                               data-toggle="modal" data-target="#modal-ch_pr">изменить</a>
                            <a class="btn active btn-dg"
                               href="<c:url value="/admin/change_delete_product/${product.id}"/>"
                               data-toggle="modal" data-target="#modal-del_pr">удалить</a>
                        </sec:authorize>
                    </c:if>
                </div>
            </div>
...
<div class="modal fade" id="modal-ch_pr">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button class="close" type="button" data-dismiss="modal">
            ...
            <div class="modal-body">
                <form roles="form" enctype="multipart/form-data" class="form-horizontal"
                      action="<c:url value="/admin/change_product"/>" method="post">
                    <input id="new-name" type="text" class="form-control" name="name" placeholder="имя">
                    ...
                    <input id="in-ch_pr" type="password" class="form-control" name="command" required
                           placeholder="введите пароль администратора">
                    <div class="btn-group modal-group">
                        <input type="reset" class="btn active btn-d" data-dismiss="modal" value="отмена"/>
                        <input type="reset" class="btn active btn-s" value=" сброс "/>
                        <input type="submit" class="btn active btn-w" value="готово"/>
                    </div>
                </form>
            </div>
        </div>
...
<div class="modal fade" id="modal-del_pr">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="modal-title">вы действительно хотите удалить товар</p>
...
<script>
    $(document).ready(function () {
        ...
        $('#modal-ch_pr').on('shown.bs.modal', function () {
            $('#in-ch_pr').focus();
        });
