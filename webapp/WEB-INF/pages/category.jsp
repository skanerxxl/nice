...
<div class="container-fluid menu-cap">
    ...
    <div class="btn-group btn-group-sm pull-right menu">
        ...
        <sec:authorize access="hasRole('USER')">
            <a href="<c:url value="/user/basket_deals_list/${0}/${'category'}"/>"
...
<div class="container container-bc">
    <ol class="breadcrumb">
        ...
        <c:if test="${page ne 'basket' && page ne 'user_deals'}">
            <li class="active li-bca"><c:out value="${nameCC}"/></li>
        </c:if>
...
<div class="container list">
    <div class="row category-mas" data-columns>
        <sec:authorize access="hasRole('ADMIN')">
            <div class="item">
                <div class="thumbnail">
                    <img class="photo_list" src="<c:url value="/img/new.jpg"/>" width="400"/>
                    <div class="caption">
                        <h3><span class="badge">new</span>категория</h3>
                        <p>создать новую категорию</p>
                        <a href="<c:url value="/admin/new_change_delete_product_category/0/''"/>"
                           class="btn active btn-p"
                           data-toggle="modal" data-target="#modal-new_catgr">создать
                            <i class="fa fa-arrow-right"></i></a>
                    </div>
                </div>
            </div>
        </sec:authorize>
        <c:forEach items="${listOfProductCategory}" var="pc">
            <div class="item">
                <div class="thumbnail">
                    <img class="photo_list" src="<c:url value="/photo/${pc.photo.id}"/>" alt="изображение отсутствует"
                         width="400"/>
                    <div class="caption">
                        <h3><span class="badge">${pc.listOfProduct.size()}</span>${pc.nameProductCategory}</h3>
                        <sec:authorize access="!hasRole('ADMIN')">
                            <a href="<c:url value="/list_product/${pc.id}/${pc.nameProductCategory}"/>"
                        ...
                        <sec:authorize access="hasRole('ADMIN')">
                            <div class="btn-group drop_catgr">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a href="<c:url value="/list_product/${pc.id}/${pc.nameProductCategory}"/>">открыть</a>
                                    </li>
                                    <li class="divider"></li>
                                    <li>
                                        <a href="<c:url value="/admin/new_change_delete_product_category/${pc.id}/${pc.nameProductCategory}"/>"
                                           data-toggle="modal" data-target="#modal-ch_catgr">изменить</a></li>
                                    <li>
                                        <a href="<c:url value="/admin/new_change_delete_product_category/${pc.id}/${pc.nameProductCategory}"/>"
                                           data-toggle="modal" data-target="#modal-del_catgr">удалить</a></li>
                                </ul>
                            </div>
                        </sec:authorize>
                    </div>
...
<%-- модальное окно - создать категорию товара --%>
<div class="modal fade" id="modal-new_catgr">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button class="close" type="button" data-dismiss="modal">
                ...
                <h4 class="modal-title">создать категорию товара</h4>
            </div>
            <div class="modal-body">
                <form roles="form" enctype="multipart/form-data" class="form-horizontal"
                      action="<c:url value="/admin/new_product_category"/>" method="post">
                    <input id="in-new_catgr" type="text" class="form-control down" name="newNamePC" required
                           placeholder="имя">
...
<%--модальное окно - изменить категорию товара--%>
<div class="modal fade" id="modal-ch_catgr">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button class="close" type="button" data-dismiss="modal">
            ...
            <div class="modal-body">
                <form roles="form" enctype="multipart/form-data" class="form-horizontal"
                      action="<c:url value="/admin/change_product_category"/>" method="post">
                    <input id="new-name" type="text" class="form-control" name="newNamePC" placeholder="новое имя">
                    <%--инпут для изображения--%>
                    <div class="fileinput fileinput-new inpput-img" data-provides="fileinput">
                        <div class="fileinput-new thumbnail">
                            <img data-src="holder.js/100%x100%"><i class="fa fa-photo fa-3x text-legend"></i>
                    ...
                    <input id="in-ch_catgr" type="password" class="form-control" name="command" required
                           placeholder="введите пароль администратора">
                    <div class="btn-group modal-group">
                        <input type="reset" class="btn active btn-d" data-dismiss="modal" value="отмена"/>
                        <input type="reset" class="btn active btn-s" value=" сброс "/>
...
<%-- модальное окно - удалить категорию товара --%>
<div class="modal fade" id="modal-del_catgr">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button class="close" type="button" data-dismiss="modal">
            ...
            <div class="modal-body">
                ...
                <form roles="form" class="form-horizontal" action="<c:url value="/admin/delete_product_category"/>"
                      method="post">
                    <input id="in-del_catgr" type="password" class="form-control" name="command" required
                           placeholder="введите пароль администратора">
                    <div class="btn-group modal-group">
                        <input type="reset" class="btn active btn-d" data-dismiss="modal" value="отмена"/>
                        <input type="reset" class="btn active btn-s" value=" сброс "/>
...
<script>
    $(document).ready(function () {
        ...
        $('#modal-new_catgr').on('shown.bs.modal', function () {
            $('#in-new_catgr').focus();
        });

        $('#modal-ch_catgr').on('shown.bs.modal', function () {
            $('#in-ch_catgr').focus();
        });

        $('#modal-del_catgr').on('shown.bs.modal', function () {
            $('#in-del_catgr').focus();
        });
    });
</script>
